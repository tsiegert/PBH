...
test_spec = pd.read_csv('spec_tools/test_spec.csv')
e_test_spec = test_spec['Energy']
f_test_spec = test_spec['Flux']
test_spec_interpol = interpol(e_test_spec,f_test_spec,bounds_error=False,fill_value=0)

def test_spec_func(energy,p):
    return p*test_spec_interpol(energy)


def fit_powerlaw_test_spec(energy,p):
    return powerlaw(energy,p[0:2]) + test_spec_func(energy,[p[2]])




def powerlaw100(energy, p):
    """
    Returns:
    Differential flux shaped as power-law function "in units of ph/cm2/s/keV"
...
# if a polwer between two energies is simulated, the integration is within the boundaries of the bins
# this reuiqres a special function to integrate:
...
def band_function(energy,p):
    """
    Returns:
    Differential flux shaped as a Band function (smoothly broken powerlaw)
    "in units of ph/cm2/s/keV"
    
    Parameters:
    :param energy: 1D-array of energies where power-law is evaluated (in keV)
    :param p:      p[0] = Normalisation (in ph/cm2/s/keV)
    :param p:      p[1] = Power-law index 1 (unitless)
    :param p:      p[2] = Cut-off energy (in keV)
    :param p:      p[3] = Power-law index 2 (unitless)
    ...
    val[hi_idx] = p[0]*((p[1]-p[3])*p[2])**(p[1]-p[3])*np.power(energy[hi_idx]/E0,p[3])*np.exp(p[3]-p[1])
    
    return val


def band_function100(energy,p):
    """
    Returns:
    Differential flux shaped as a Band function (smoothly broken powerlaw)
    "in units of ph/cm2/s/keV"
...
def fit_powerlaw_Gauss(energy,p):
    ...
    line = Gaussian(energy,[p[2],511,5])
    return plaw+line


def powerlaw_Gauss(energy,p):
    plaw = powerlaw(energy,p[0:2])
    line = Gaussian(energy,p[2:])
    return plaw+line
